...
vector<vector<int>> makeTree(int n, vector<vector<int>> &links) {
    vector<vector<int>> tree(n, vector<int>(0));//관계 트리
    for (int i = 0; i < links.size(); i++)
        tree[links[i][0]].push_back(links[i][1]);//팀장 노드에 팀원 값 추가
    return tree;//관계 트리 반환
}

int fillDp(vector<int> &sales, vector<vector<int>> &tree, int node, bool flag) {
    if (dp[node][flag] != -1) //이미 탐색했던 상태
        return dp[node][flag];//dp에 저장된 탐색값 반환
    ...
    if (tree[node].empty()) //말단직원
        return dp[node][flag] = flag ? sales[node - 1] : 0;//참석하면 말단직원의 매출액 반환, 불참이면 0
    ...
    for (int i = 0; i < tree[node].size(); i++) {
        ...
        if (attend < absent) //부하직원이 한 명이라도 참석한 경우
            attend_flag = true;//참석 여부 true로 
        diff = min(diff, attend - absent); //모든 팀원이 불참했을 때를 대비
    }
    if (flag) //팀장 참석
        return dp[node][flag] += sales[node - 1];//dp + 팀장의 매출액 
...
int solution(vector<int> sales, vector<vector<int>> links) {
    int v = sales.size();//전체 직원 수 v
    dp.assign(v + 1, vector<int>(2, -1));//dp 연산 위한 dp 배열
    vector<vector<int>> tree = makeTree(v + 1, links); //트리 만들기

    //CEO가 참석하는 경우, CEO가 참석하지 않는 경우
    return min(fillDp(sales, tree, 1, true), fillDp(sales, tree, 1, false));//매출액의 합 최소값
}
 ...
 */
int main() {
    vector<int> sales = {14, 17, 15, 18, 19, 14, 13, 16, 28, 17};//하루 평균 매출액
    vector<vector<int>> links = {{10, 8},//팀장-팀원 관계 나타내는 2차원 배열, [a,b] a 팀장 직원번호, b 팀원 직원번호
                                 {1,  9},
                                 {9,  7},
                                 {5,  4},
                                 {1,  5},
                                 ...
                                 {1,  3},
                                 {10, 2}};
    int ans = solution(sales, links);//매출액 최소값 연산
    cout << ans;//최소값 출력
}