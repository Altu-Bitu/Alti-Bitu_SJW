...
void maxConsul(int n, vector<ci> &consulting, int start, int sum) {
    ...
    ans = max(ans, sum); //최대 이익과 지금까지의 이익 중 더 큰 값을 최대 이익으로

    for (int i = start; i < n; i++) //i번째 날을 상담하는 경우
        maxConsul(n, consulting, i + consulting[i].first, sum + consulting[i].second); //최대 이익 연산
}
...
int main() {
    ...
    //입력
    cin >> n; //퇴사까지 남은 날 입력
    vector<ci> consulting(n); //(상담시간, 이익) 입력을 위한 vector 선언

    for (int i = 0; i < n; i++)
        cin >> consulting[i].first >> consulting[i].second; //first: 상담 시간, second: 이익 입력

    //연산
    maxConsul(n, consulting, 0, 0); //최대 이익 연산
