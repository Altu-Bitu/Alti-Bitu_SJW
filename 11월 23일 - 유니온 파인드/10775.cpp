 ...
 */
int main() {
    int g, p, gi;//게이트 수 g, 비행기 수 p

    //입력
    cin >> g >> p;//게이트 수, 비행기 수 입력
    parent.assign(g + 1, -1);//parent 크기 g+1, -1로 초기화
    for (int i = 0; i < p; i++) {
        cin >> gi;//i번째 비행기 도킹할 게이트 번호 입력

        //연산
        if (!unionGate(gi)) { //도킹 불가능
            cout << i;//도킹시킬 수 있는 최대 비행기 수 출력
            return 0;
        }
    }

    //출력
    cout << p;//비행기 다 도킹 가능할 때
}