...
vector<bool> visited_recur; //재귀함수에서 방문 여부 표시하는 벡터
...
void dfsRecur(int v) {
    ...
    for (int i = 0; i < adj_list[v].size(); i++)//인접리스트 v의 사이즈만큼 for문
        dfsRecur(adj_list[v][i]);//재귀함수 호출
}

//DFS : 깊이 우선 탐색
vector<int> dfs(int n, int v) {
    stack<int> st; //탐색할 노드 넣어두는 스택
    vector<bool> visited(n + 1, false); //방문 노드 체크
    vector<int> result; //탐색순으로 노드 저장

    st.push(v); //처음 탐색 노드 삽입
    visited[v] = true; //방문 체크
    result.push_back(v); //방문한 노드 번호 result에 저장
    ...
    return result;//탐색순으로 저장한 노드 벡터 반환
}

//BFS : 너비 우선 탐색
vector<int> bfs(int n, int v) {
    queue<int> q;//탐색할 노드 저장할 큐
    vector<bool> visited(n + 1, false); //방문 노드 체크
    ...
    while (!q.empty()) {
        ...
        q.pop();//큐에서 원소 제거
        result.push_back(node);//result 벡터에 탐색하는 노드 삽입
        //인접행렬로 검사
        for (int i = 1; i <= n; i++) { //모든 노드에 대해
            if (adj_matrix[node][i] && !visited[i]) { //연결이 되어 있고, 처음 방문이라면
                visited[i] = true; //방문 체크
...
int main() {
    ...
    adj_matrix.assign(n + 1, vector<bool>(n + 1, false));//인접 행렬 크기 n+1 2차원, false로 초기화 
    adj_list.assign(n + 1, vector<int>());//인접 리스트 크기 n+1
    while (m--) {//간선 수 0될때까지
        cin >> a >> b;//간선 정보 입력
        //인접행렬
        adj_matrix[a][b] = adj_matrix[b][a] = true;//인접 행렬 true로 바꿔서 연결 정보 저장
        //인접리스트
        adj_list[a].push_back(b);//a의 인접리스트에 b 저장
        adj_list[b].push_back(a);//b의 인접리스트에 a 저장
    }

    //인접리스트 구현 시, 노드 번호가 작은 것부터 방문하기 위해 정렬해야 함
    for (int i = 1; i <= n; i++)
        sort(adj_list[i].begin(), adj_list[i].end());//각 노드의 인접리스트 정렬

    //DFS - stack
    //result[0] = dfs(n, v);
    //DFS - 재귀함수
    visited_recur.assign(n + 1, false);//방문 여부 표시 벡터 사이즈 n+1, 모두 false로 초기화
    dfsRecur(v);//dfs 재귀함수 실행
    //BFS - queue
    result[1] = bfs(n, v);//bfs 함수 실행
