...
int dfs(int cr, int cc) {
    ...
    for (int i = 0; i < 4; i++) {
        int nr = cr + dr[i];//row, col 값 바꿔서 상하좌우 위치 이동
        int nc = cc + dc[i];
...
int main() {
    //입력
    cin >> n >> m;//지도 가로 n, 세로 m 입력
    board.assign(n, vector<int>(m, 0));//n*m 크기인 2차원 배열 모두 0으로
    dp.assign(n, vector<int>(m, -1)); //이미 탐색한 값이 0일 수도 있기 때문에 구분하기 위해 -1로 초기화
    for (int i = 0; i < n; i++)
        for (int j = 0; j < m; j++)
            cin >> board[i][j];//각 지점의 높이 입력

    //연산 & 출력
    cout << dfs(0, 0);//이동 가능한 경로 수 연산 및 출력
