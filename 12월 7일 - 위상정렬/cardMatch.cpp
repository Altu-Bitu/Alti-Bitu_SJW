...
vector<vector<ci>> findCard(vector<vector<int>> &board) { //존재하는 모든 카드의 위치를 리턴하는 함수
    ...
    for (int i = 0; i < SIZE; i++) {
        for (int j = 0; j < SIZE; j++) {
            cnt = max(cnt, board[i][j]);//cnt,board[i][j] 중 최댓값 cnt에 저장
            if (board[i][j]) //카드라면 위치 저장
                card_pos[board[i][j]].emplace_back(i, j);//최대 카드 수 벡터에 요소 추가
        }
    }
    card_pos.resize(cnt + 1); //실제 존재하는 카드의 수만큼 크기 조절
    return card_pos;//모든 카드 위치 반환 
}
...
pair<int, int> ctrl(int row, int col, int dir, vector<vector<int>> &tmp) { //컨트롤로 이동하는 좌표를 리턴하는 함수
    while (true) {
        ...
        if (row < 0 || row >= SIZE || col < 0 || col >= SIZE) //해당 방향에 카드가 하나도 없다면 그 방향의 가장 마지막 칸으로 이동
            return make_pair(row - dr[dir], col - dc[dir]);//이동할 좌표 반환
        if (tmp[row][col] != 0) //누른 키 방향에 있는 가장 가까운 카드
            return make_pair(row, col);//이동할 좌표 반환
    }
}

int bfs(int r1, int c1, int r2, int c2, vector<vector<int>> &tmp) { //현재 커서에서 목표 카드로 이동하는 가장 적은 비용을 리턴하는 함수
    vector<vector<int>> visited(SIZE, vector<int>(SIZE, 0));//방문 표시
    queue<pair<int, int>> q;//탐색할 노드 큐
    visited[r1][c1] = 1; //반드시 엔터를 누르게 될 것. 엔터를 미리 눌렀다 가정하고 1로 표시
    q.push({r1, c1});//큐에 삽입

    while (!q.empty()) {//큐 빌때까지
        int row = q.front().first;//큐의 맨 앞 원소 쌍의 첫번쨰 원소 row에 저장
        int col = q.front().second;//큐의 맨 앞 원소 쌍의 두번쨰 원소 col에 저장
        int dist = visited[row][col];//방문 위치의 값 dist에 저장
        q.pop();//큐 원소 제거

        if (row == r2 && col == c2) //목표 카드에 도달했다면
            return dist;//방문 위치 값 반환

        for (int i = 0; i < 4; i++) { //컨트롤 입력 이동
            pair<int, int> np = ctrl(row, col, i, tmp);//컨트롤 이동 좌표 np에 저장
            if (visited[np.first][np.second])//이미 방문했으면
                continue;//반복문 처음으로
            visited[np.first][np.second] = dist + 1;//방문 안했으면 dist+1 값 visited에 저장
            q.push(np);//큐에 np 삽입
        }

        for (int i = 0; i < 4; i++) { //방향키 입력 이동
            int nr = row + dr[i], nc = col + dc[i];//행, 열 이동
            if (nr < 0 || nr >= 4 || nc < 0 || nc >= 4 || visited[nr][nc])//범위 벗어나면
                continue;//반복문 처음으로
            visited[nr][nc] = dist + 1;//dist+1 값 visited[nr][nc]에 저장
            q.push({nr, nc});//큐에 {nr,nc} 삽입
        }
    }
...
int matchCard(int bit, int num, int r, int c, vector<int> &seq, vector<vector<ci>> &cards, vector<vector<int>> tmp){ //조합에 대해 카드를 매칭하는 함수
    int ans = 0;//현재 커서에서 목표 카드까지 이동하는 비용
    for (int i = 0; i < num; i++) {
    ...
    return ans;//현재 커서에서 목표 카드까지 이동하는 비용 반환
}

int solution(vector<vector<int>> board, int r, int c) {
    int answer = 1e9;//키 조작 횟수 최솟값 answer
    ...
    for (int i = 0; i < card_cnt; i++)
        seq[i] = i + 1;//seq[i]의 값 i+1로 저장

    do { //가능한 모든 카드 순서에 대해
        for (int bit = 0; bit < (1 << card_cnt); bit++) { //0, 1번째 카드 중 어떤 카드를 먼저 뒤집을지 결정
            answer = min(answer, matchCard(bit, card_cnt, r, c, seq, cards, board));//최솟값 갱신
        }
    } while (next_permutation(seq.begin(), seq.end()));
    return answer;//키 조작 횟수 최솟값 반환
}
