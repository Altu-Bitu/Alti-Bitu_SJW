...
vector<string> topologicalSort(set<string> &item) {
    vector<string> result;//구매해야하는 아이템 순서 저장하는 result
    priority_queue<string, vector<string>, greater<>> pq, pq_temp;//탐색 위한 우선순위 큐 pq,pq_temp

    for (auto iter = item.begin(); iter != item.end(); iter++) {
        if (indegree.find(*iter) == indegree.end()) //진입차수가 존재하지 않는다면(0 이라면)
            pq.push(*iter);//우선순위 큐에 삽입
    }
    while (!pq.empty()) {//우선순위 큐 빌 떄까지 반복
        string node = pq.top();//우선순위큐 pq에서 가장 앞 원소 node에 저장
        pq.pop();//pq에서 제거

        result.push_back(node); //현재 정점 순서에 삽입
        for (int i = 0; i < graph[node].size(); i++) {
            string next_node = graph[node][i];//next_node에 graph[node][i] 값 저장
        ...
        if (pq.empty() && !pq_temp.empty()) //이전에 진입차수가 0이였던 정점을 모두 탐색했고, 새로 탐색할 정점이 있다면
            swap(pq, pq_temp);//pq, pq_temp 교환
    }
    return result;//아이템 구매 순서 반환
}
...
int main() {
    ios_base::sync_with_stdio(false);//입출력 속도 빠르게
    cin.tie(NULL);
    cout.tie(NULL);

    set<string> item; //존재하는 아이템(정점)
    int n;//아이템 사이의 관계 수 n
    string a, b;//아이템 이름 a,b

    //입력
    cin >> n;//아이템 사이의 관계 수 입력
    while (n--) {
        cin >> a >> b; //a < b
        indegree[b]++; //진입차수 증가
    ...
    //연산
    vector<string> result = topologicalSort(item);//구매해야 하는 아이템 순서대로 연산

    //출력
    if (result.size() != item.size()) {//모든 아이템을 구매할 수 없는 경우
        cout << "-1\n";//-1 출력
        return 0;
    }
    for (int i = 0; i < result.size(); i++)//모든 아이템을 구매 가능한 경우
        cout << result[i] << '\n';//구매해야하는 순서대로 아이템 출력
